...
class DeepSC(nn.Module):
    ...
    def forward(self, src, trg=None, src_mask=None, trg_mask=None, look_ahead_mask=None):
        """
        Args:
            src: Source input tensor (batch_size, src_seq_len)
            trg: Target input tensor (batch_size, trg_seq_len) — optional during inference
            src_mask: Source mask for padding
            trg_mask: Target mask for padding
        ...
        # Channel encoding (like modulation in comms)
        channel_out = self.channel_encoder(enc_output)
        
        # Channel decoding (like demodulation)
        channel_decoded = self.channel_decoder(channel_out)
        
        if trg is not None:
            # Decoder step (training or teacher-forced inference)
            output = self.decoder(trg, channel_decoded, look_ahead_mask, trg_mask)
        else:
            # In pure inference, you might only output channel-decoded encoder output
            output = channel_decoded
